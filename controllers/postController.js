...
exports.createPost = async (req, res) => {
  try {
    ...
    // Create and save the post to MongoDB
    ...
    // --- FIREBASE PUSH NOTIFICATION (NON-BLOCKING) ---
    // We do not 'await' this so the student gets an immediate response
    const notificationMessage = {
      notification: {
        title: 'New Community Post! 📢',
        // Safe truncation for notification display
        body: content.length > 100 ? content.substring(0, 97) + "..." : content, 
      },
      topic: 'community_posts', // Matches the subscription in Flutter
      data: {
        click_action: 'FLUTTER_NOTIFICATION_CLICK',
        screen: 'community_page', // Deep link identifier for Flutter
    ...
    admin.messaging().send(notificationMessage)
      ...
      .catch((error) => {
        console.error('FCM Error (Post saved, but notification failed):', error);
      });
    // ------------------------------------------------
...
exports.toggleLike = async (req, res) => {
  try {
    ...
    let update;
    if (isLiked) {
      // Atomic Unlike: Decrement count and pull ID in one operation
      update = { $inc: { likesCount: -1 }, $pull: { likedBy: userId } };
    } else {
      // Atomic Like: Increment count and push ID in one operation
